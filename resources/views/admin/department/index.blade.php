...
{{--            {!! Breadcrumbs::render('department'); !!}--}}
                            ...
                            <tbody>
                                @foreach($list as $v)
                                    <tr>
                                        ...
                                        <td>{{$v['pid']}}</td>
                                        <td>
                                            <span class="btn btn-xs btn-info" title="修改信息" onclick="updateDepartment('{{$v['id']}}')" data-toggle="modal" data-target=".bs-example-modal-md"><i class="fa fa-wrench"></i> 修改</span>
                                            <span class="btn btn-xs btn-danger" title="删除类别" onclick="deleteDepartment('{{$v['id']}}')"><i class="fa fa-trash-o" ></i> 删除</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <script type="text/javascript">
        ...
        function add() {
            ...
            $.ajax({
                url: "{{ url('admin/department/create') }}",
                type: 'GET',
                dataType: 'HTML',
                cache:false,
                beforeSend: function () {
        ...
        function deleteDepartment(id) {
            deleteItems(id,"确定删除该部门吗");
        }

        $("#simple-search").on('click',function () {
            window.location.href = "{{url('admin/department')}}?search="+$("#search-text").val();
        });
